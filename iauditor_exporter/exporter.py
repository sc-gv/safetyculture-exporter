    ...
    if list_of_audits is not None:
        ...
        for export_format in settings[EXPORT_FORMATS]:
            if export_format == 'sql':
                ...
                #             settings[SQL_TABLE]))
                #     sys.exit(0)
        tqdm_list = tqdm(list_of_audits['audits'])
        for audit in tqdm_list:
            audit_id = audit['audit_id']
            tqdm_list.set_description('Processing audit (' + str(export_count) + '/' + str(export_total) + ')' + ' - ' + str(audit_id))
            logger.debug('Processing audit (' + str(export_count) + '/' + str(export_total) + ')')
...
def process_audit(logger, settings, sc_client, audit, get_started):
    ...
    if not check_if_media_sync_offset_satisfied(logger, settings, audit):
        return
    audit_id = audit['audit_id']
    logger.debug('downloading ' + audit_id)
